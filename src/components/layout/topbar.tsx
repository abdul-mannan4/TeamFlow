...
import UnderConstructionModal from "../SharedComponents/UnderConstruction/underConstructionModal";
...
export default function Topbar({ onMenuClick }: { onMenuClick: () => void }) {
  ...
  const handleOpenUnderConstruction = (feature: string) => {
    setModalState({
      isOpen: true,
      featureName: feature,
    });
    ...
    <>
      <div className="flex px-4 sm:px-6 2xl:px-8 gap-3 sm:gap-4 bg-white items-center justify-between h-full w-full">
        ...
        <div className="flex gap-3 sm:gap-6 items-center shrink-0">
          <button
            ...
            onClick={() => handleOpenUnderConstruction("Notifications")}
          ...
          <button
            type="button"
            onClick={() => handleOpenUnderConstruction("Admin Profile & Settings")}
            className="flex gap-2.5 items-center cursor-pointer hover:opacity-90 transition-opacity bg-transparent border-0 p-0 text-left"
          >
      ...
      <UnderConstructionModal
        ...
        setIsOpen={(open) =>
          setModalState((prev) => ({
            ...prev,
            isOpen: typeof open === "function" ? open(prev.isOpen) : open,
          }))
        }
      />
    </>
